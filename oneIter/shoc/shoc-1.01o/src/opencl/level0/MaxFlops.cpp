...
struct _benchmark_type {
  ...
  {"Add1", "s", "data[gid]", "10.0-$", 1, 240, 20, 1, 1024, 1024, 4},
  {"Add2", "s", "data[gid]", "10.0-$", 2, 120, 20, 1, 1024, 1024, 4},
  {"Add4", "s", "data[gid]", "10.0-$", 4, 60, 20, 1, 1024, 1024, 4},
  {"Add8", "s", "data[gid]", "10.0-$", 8, 30, 20, 1, 1024, 1024, 4},
  {"Mul1", "s", "data[gid]-data[gid]+0.999", "$*$*1.01", 1, 200, 20, 2, 1024, 1024, 4},
  {"Mul2", "s", "data[gid]-data[gid]+0.999", "$*$*1.01", 2, 100, 20, 2, 1024, 1024, 4},
  {"Mul4", "s", "data[gid]-data[gid]+0.999", "$*$*1.01", 4, 50, 20, 2, 1024, 1024, 4},
  {"Mul8", "s", "data[gid]-data[gid]+0.999", "$*$*1.01", 8, 25, 20, 2, 1024, 1024, 4},
  {"MAdd1", "s", "data[gid]", "10.0-$*0.9899", 1, 240, 20, 2, 1024, 1024, 4},
  {"MAdd2", "s", "data[gid]", "10.0-$*0.9899", 2, 120, 20, 2, 1024, 1024, 4},
  {"MAdd4", "s", "data[gid]", "10.0-$*0.9899", 4, 60, 20, 2, 1024, 1024, 4},
  {"MAdd8", "s", "data[gid]", "10.0-$*0.9899", 8, 30, 20, 2, 1024, 1024, 4},
  {"MulMAdd1", "s", "data[gid]", "(3.75-0.355*$)*$", 1, 160, 20, 3, 1024, 1024, 4},
  {"MulMAdd2", "s", "data[gid]", "(3.75-0.355*$)*$", 2, 80, 20, 3, 1024, 1024, 4},
  {"MulMAdd4", "s", "data[gid]", "(3.75-0.355*$)*$", 4, 40, 20, 3, 1024, 1024, 4},
  {"MulMAdd8", "s", "data[gid]", "(3.75-0.355*$)*$", 8, 20, 20, 3, 1024, 1024, 4},
  {0, 0, 0, 0, 0, 0, 0, 0, 0}
};
                  ...
                  OptionParser &op)
{
    ...
    int aIdx = 0;
    float repeatF = 1.0f;
    // Find the index of the MulMAdd2 benchmark
    while ((aTests!=0) && (aTests[aIdx].name!=0) && 
        strcmp(aTests[aIdx].name,"MulMAdd2"))
    {   
       aIdx += 1;
    }
    if (aTests && aTests[aIdx].name)  // we found a benchmark with that name
    {
       ...
       {
           hostMem[j] = hostMem[numFloatsMax-j-1] = (float)(drand48()*10.0);
       }
...
generateKernel (ostringstream &oss, struct _benchmark_type &test, const char* tName, const char* header)
{
    ...
    for (int ss=2 ; ss<=test.numStreams ; ++ss)
    {
        oss << ", " << test.indexVar << ss << " = ";
        if (ss==2) oss << "10.0 - " << test.indexVar;
        else if (ss%2==0) oss << test.indexVar << "2";
             else oss << test.indexVar;
    }
    oss << ";\n";
    ...
    for (int uu=0 ; uu<test.numUnrolls ; ++uu) {
        ...
        int pos = -1;
        string lVar = string(test.indexVar);
        while ((pos=opCode.find("$"))!=(-1))
            opCode.replace(pos,1,lVar);
        oss << "  " << test.indexVar << "=" << opCode << ";";
       
        for (int ss=2 ; ss<=test.numStreams ; ++ss)
        {
            opCode = string(test.opFormula);
            pos = -1;
