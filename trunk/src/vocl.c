...
static void checkSlaveProc()
{
	...
	if (slaveCreated == 0)
	{
		...
		MPI_Comm_spawn("/home/scxiao/workplace/trunk/vocl/vocl_proxy", MPI_ARGV_NULL, np, MPI_INFO_NULL, 0, MPI_COMM_WORLD, &slaveComm, errCodes);
		slaveCreated = 1;
                     ...
                     cl_event *         event)
{
	...
	for (i = 0; i <= bufferNum; i++)
	{
		...
		MPI_Isend((void *)((char*)ptr + i * VOCL_WRITE_BUFFER_SIZE), bufferSize, MPI_BYTE, 0, 
				  VOCL_WRITE_TAG + bufferIndex, slaveComm, getWriteRequestPtr(bufferIndex));
                       ...
                       cl_event *       event)
{
	...
	if (num_events_in_wait_list > 0)
	{
		MPI_Isend((void *)event_wait_list, sizeof(cl_event) * num_events_in_wait_list, MPI_BYTE,
				  0, ENQUEUE_ND_RANGE_KERNEL1, slaveComm, request+(requestNo++));
	}
...
clFinish(cl_command_queue hInCmdQueue)
{
	...
	MPI_Isend(&tmpFinish, sizeof(tmpFinish), MPI_BYTE, 0,
			 FINISH_FUNC, slaveComm, &request1);
	MPI_Wait(&request1, status);
	MPI_Irecv(&tmpFinish, sizeof(tmpFinish), MPI_BYTE, 0,
			 FINISH_FUNC, slaveComm, &request2);
