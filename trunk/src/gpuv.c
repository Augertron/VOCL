...
void checkSlaveProc()
{
	...
	{
		MPI_Init(NULL, NULL);
		MPI_Comm_spawn("/home/shucaixiao/workplace/trunk/lib/slave_process", MPI_ARGV_NULL, np, MPI_INFO_NULL, 0, MPI_COMM_WORLD, &slaveComm, errCodes);
		slaveCreated = 1;
		char hostName[200];
		int  len;
		...
		wrtCRequest = (MPI_Request *)malloc(sizeof(MPI_Request) * (MAX_WRITE_TAG-MIN_WRITE_TAG));
		wrtDRequest = (MPI_Request *)malloc(sizeof(MPI_Request) * (MAX_WRITE_TAG-MIN_WRITE_TAG));
		readCRequest = (MPI_Request *)malloc(sizeof(MPI_Request) * (MAX_READ_TAG-MIN_READ_TAG));
		readDRequest = (MPI_Request *)malloc(sizeof(MPI_Request) * (MAX_READ_TAG-MIN_READ_TAG));
		tmpEnqueueWriteBuffer = (struct strEnqueueWriteBuffer *)malloc((MAX_WRITE_TAG-MIN_WRITE_TAG) * 
					sizeof(struct strEnqueueWriteBuffer));
		tmpEnqueueReadBuffer = (struct strEnqueueReadBuffer *)malloc((MAX_READ_TAG-MIN_READ_TAG) * 
                     ...
                     cl_event *         event)
{
	...
	//send parameters to remote node
	MPI_Isend(&tmpEnqueueWriteBuffer[writeIndex], sizeof(struct strEnqueueWriteBuffer), MPI_BYTE, 0, 
			 ENQUEUE_WRITE_BUFFER, slaveComm, wrtCRequest+writeIndex);
	//MPI_Request_free(&request);
	if (num_events_in_wait_list > 0)
	{
		MPI_Isend((void *)event_wait_list, sizeof(cl_event) * num_events_in_wait_list, MPI_BYTE, 0,
				 tmpEnqueueWriteBuffer[writeIndex].tag, slaveComm, request+(requestNo++));
		//MPI_Request_free(&request);
	}

	//MPI_Isend((void *)ptr, cb, MPI_BYTE, 0, ENQUEUE_WRITE_BUFFER+nbWriteTag, slaveComm, request+(requestNo++));
	MPI_Isend((void *)ptr, cb, MPI_BYTE, 0, tmpEnqueueWriteBuffer[writeIndex].tag, slaveComm, wrtDRequest+writeIndex);
	//MPI_Request_free(&request);

	if (blocking_write == CL_TRUE)
	{
		MPI_Irecv(&tmpEnqueueWriteBuffer[writeIndex], sizeof(struct strEnqueueWriteBuffer), MPI_BYTE, 0, 
                    ...
                    cl_event *          event)
{
	...
	//send parameters to remote node
	MPI_Isend(&tmpEnqueueReadBuffer[readIndex], sizeof(struct strEnqueueReadBuffer), MPI_BYTE, 0, 
			 ENQUEUE_READ_BUFFER, slaveComm, readCRequest+readIndex);
	//MPI_Request_free(request);
	if (num_events_in_wait_list > 0)
	{
		MPI_Isend((void *)event_wait_list, sizeof(cl_event) * num_events_in_wait_list, MPI_BYTE, 0,
	...
	{
		MPI_Irecv(&tmpEnqueueReadBuffer[readIndex], sizeof(struct strEnqueueReadBuffer), MPI_BYTE, 0, 
	...
	else //non blocking read
	{
		...
		MPI_Irecv(ptr, cb, MPI_CHAR, 0, nbReadTag+ENQUEUE_READ_BUFFER, slaveComm, readDRequest+readIndex);
                   ...
                   cl_int *         errcode_ret)
{
	...
	{
		MPI_Isend((void *)event_wait_list, sizeof(cl_event) * num_events_in_wait_list, MPI_BYTE, 0,
				 ENQUEUE_MAP_BUFF_FUNC1, slaveComm, request+(requestNo++));
	}
                    ...
                    cl_event *          event)
{
	...
	{
		MPI_Isend((void *)event_wait_list, sizeof(cl_event) * num_events_in_wait_list, MPI_BYTE, 0,
				 ENQ_COPY_BUFF_FUNC1, slaveComm, request+(requestNo++));
	}
						...
						cl_event *       event)
{
	...
	{
		MPI_Isend((void *)event_wait_list, sizeof(cl_event) * num_events_in_wait_list, MPI_BYTE, 0,
				 ENQ_UNMAP_MEMOBJ_FUNC1, slaveComm, request+(requestNo++));
	}
