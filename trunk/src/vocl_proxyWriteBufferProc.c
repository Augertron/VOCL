...
void finalizeWriteBuffer()
{
	...
	for (i = 0; i < VOCL_PROXY_WRITE_BUFFER_NUM; i++)
	{
		if (writeBufferInfo[i].dataPtr) free(writeBufferInfo[i].dataPtr);
		if (writeBufferInfo[i].eventWaitList) free(writeBufferInfo[i].eventWaitList);
	}
...
static void getAllPreviousReadBuffers(int writeIndex)
{
	...
	for (i = startIndex; i < endIndex; i++)
	{
		...
		{
			writeBufferInfo[writeIndex].sendProcInfo.readBufferIndex[writeBufferInfo[writeIndex].sendProcInfo.toBeProcessedNum++] = index;
