...
{
	int i;
	for (i = 0; i < voclAppNo; i++)
	{
		MPI_Comm_free(appComm+i);
		MPI_Comm_free(appCommData+i);
	}
	free(conMsgRequest);
	free(appComm);
	free(appCommData);
...
int voclGetAppIndex()
{
	...
	{
		voclAppNum *= 2;
		conMsgRequest = (MPI_Request *)realloc(conMsgRequest, voclAppNum * CMSG_NUM * sizeof(MPI_Request));
		...
		conMsgBuffer = (CON_MSG_BUFFER *)realloc(conMsgBuffer, voclAppNum * CMSG_NUM * sizeof(CON_MSG_BUFFER));
	}
