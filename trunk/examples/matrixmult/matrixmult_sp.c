...
int main(int argc, char **argv)
{
	...
	for (iterationNo = 0; iterationNo < numIterations; iterationNo++)
	{
		...
		err |= clSetKernelArg(hKernel, 3, sizeof(cl_float) * BLOCK_SIZE * (BLOCK_SIZE + 1), (void *)NULL);
		err |= clSetKernelArg(hKernel, 4, sizeof(cl_float) * BLOCK_SIZE * (BLOCK_SIZE + 1), (void *)NULL);
