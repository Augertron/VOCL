...
void RunTest(string testName, ResultDatabase &resultDB, OptionParser &op)
{
    ...
    T* h_odata;
    CUDA_SAFE_CALL(cudaMallocHost((void**)&h_odata, numBlocks * sizeof(T)));

    T* d_odata;
    CUDA_SAFE_CALL(cudaMalloc((void**)&d_odata, numBlocks * sizeof(T)));
    ...
    for (int k=0; k<passes; k++)
    {
        ...
        {
            reduce<T><<<numBlocks,numThreads>>>(d_idata, d_odata, size);
        ...
        double avgTime = totalReduceTime / (double)iters;
        avgTime *= 1.e-3; // convert to seconds
