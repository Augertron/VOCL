           ...
           const unsigned int sizes[], ResultDatabase &resultDB)
{
   ...
   for (j=sIdx ; j<nsizes ; ++j)
   {
       ...
       for (i=0 ; i<passes ; ++i)
       {
            resultDB.AddResult("ImageLinearAccess", sizeStr, "GB/sec", FLT_MAX);
            resultDB.AddResult("ImageRandAccess",   sizeStr, "GB/sec", FLT_MAX);
            resultDB.AddResult("ImageCacheAccess",  sizeStr, "GB/sec", FLT_MAX);
       }
   }
}
...
void TestImageMemory(cl_context ctx,
    ...
    for (int j = 0; j < nsizes; j++)
    {
        ...
        for (int p = 0; p < passes; p++)
        {
            ...
            resultDB.AddResult("ImageLinearAccess", sizeStr, "GB/sec",
                    speed);
            ...
            sprintf(sizeStr, "% 6dkB", size / 1024);
            resultDB.AddResult("ImageCacheAccess", sizeStr, "GB/sec",
                    speed);
            ...
            speed = (double)kernelRepFactor * (double)size / t;
            sprintf(sizeStr, "% 6dkB", size / 1024);
            resultDB.AddResult("ImageRandAccess", sizeStr, "GB/sec",
                    speed);
        }
        delete[] h_in;
        delete[] h_out;
