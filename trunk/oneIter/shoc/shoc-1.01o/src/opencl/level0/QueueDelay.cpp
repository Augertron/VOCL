                  ...
                  OptionParser &op)
{
	...
	//---------------------------------
	 ...
	 tempTime += 1000.0 * (t2.tv_sec - t1.tv_sec) + (t2.tv_usec - t1.tv_usec) / 1000.0;
	 //----------------------------------------------
	   ...
	   //debug-----------------------------------------------------------
		resultDB.AddResult("totalDelay", "1 kernel", "ms",
						  (tempTime / ((double)reps * 4.0)) );
	   //----------------------------------------------------------------
       total = 0.0;
    }
	...
	//---------------------------------
	 ...
	 //----------------------------------------------
       ...
       resultDB.AddResult("SSDelay", "2 Kernels", "ms", 
                          (total / ((double)reps * 4.0)) / 1.0e6);
	   //debug-----------------------------------------------------------
		resultDB.AddResult("totalDelay", "2 kernels", "ms",
						  (tempTime / ((double)reps * 4.0)) );
	   //----------------------------------------------------------------
       total = 0.0;
    }
	...
	//---------------------------------
	 ...
	 //----------------------------------------------
       ...
       resultDB.AddResult("SSDelay", "4 Kernels", "ms", 
                          (total / ((double)reps * 4.0)) / 1.0e6);
		resultDB.AddResult("totalDelay", "4 kernels", "ms",
						  (tempTime / ((double)reps * 4.0)) );
	   //----------------------------------------------------------------
       total = 0.0;
       total = 0.0;
