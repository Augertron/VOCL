...
static VCLX_Mutex *voclWinLockers = NULL;
...
void voclWinInfoInitialize()
{
	...
	voclWinLockers = (VCLX_Mutex *)malloc(sizeof(VCLX_Mutex) * totalVoclWinInfoNum);
...
void voclWinInfoFree(int proxyIndex)
{
	...
	//VCLX_Mutex_destroy(voclWinLockers[proxyIndex]);
	VCLX_Mutex_free(&voclWinLockers[proxyIndex]);
...
void voclAddWinInfo(MPI_Comm comm, int proxyRank, int proxyIndex, char *serviceName)
{
	...
	//voclWinLockers[proxyIndex] = VCLX_Mutex_create(2, winComm);
	VCLX_Mutex_create(2, winComm, &voclWinLockers[proxyIndex]);
...
{
	VCLX_Mutex_lock(voclWinLockers[proxyIndex], 0, 0);
	return;
}

void voclMigrationMutexUnlock(int proxyIndex)
{
	VCLX_Mutex_unlock(voclWinLockers[proxyIndex], 0, 0);
	return;
}

void voclConMsgMutexLock(int proxyIndex)
{
	VCLX_Mutex_lock(voclWinLockers[proxyIndex], 1, 0);
	return;
}

void voclConMsgMutexUnlock(int proxyIndex)
{
	VCLX_Mutex_unlock(voclWinLockers[proxyIndex], 1, 0);
	return;
}
