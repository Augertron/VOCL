...
do
	echo "***************** iter = ${iteration_sp[i]} matrix = ${matrixSize[j]} * ${matrixSize[j]} *****************" >> bb48_mm_result.txt
#	proxyfile="-env PROXY_HOST_FILE=hostfile$j.txt"
	proxyfile=""
	options="$preload $dpm $namepub $proxy -hosts $hosts $bind -np 1"
	echo "mpiexec.hydra $options ./matrixmult_sp ${matrixSize[j]} ${iteration_sp[j]} ${gpu[j]} >> bb48_mm_result.txt"
	/home/liyan/mvapich2-1.8/bin/mpiexec.hydra $options ./vocl_lib/bin/matrixmult_sp ${matrixSize[j]} ${iteration_sp[j]} ${gpu[j]} >> bb48_mm_result.txt	
done
