...
do
	echo "***************** new matrix = ${matrixSize[j]} * ${matrixSize[j]} *****************" >> bb44_mm_result.txt
#	proxyfile="-env PROXY_HOST_FILE=hostfile$j.txt"
	...
	/home/liyan/mvapich2-1.8/bin/mpiexec.hydra $options ./vocl_lib/bin/matrixmult_sp ${matrixSize[j]} ${iteration_sp[j]} ${gpu[j]} >> ib_bb47_mm_result.txt	
done
