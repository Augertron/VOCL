...
iteration_sp=(10 10 10 10 10 10 10)
...
#	echo "mpiexec.hydra $options ./matrixmult_sp ${matrixSize[j]} ${iteration_sp[j]} ${gpu[j]} >> remote_cpu_ib_bb48_mm_result.txt"
	/home/liyan/mvapich2-1.8/bin/mpiexec.hydra $options ./matrixmult_sp ${matrixSize[j]} ${iteration_sp[j]} ${gpu[j]} >> remote_cpu_ib_bb48_mm_result.txt
#	/home/liyan/mvapich2-1.8/bin/mpiexec.hydra $options ./matrixmult_sp ${matrixSize[j]} ${iteration_sp[j]} ${gpu[j]} 
