...
void voclProxyGetDeviceIDs()
{
	...
	{
		retCode |= clGetDeviceIDs(platformIDs[i], CL_DEVICE_TYPE_CPU, 0, NULL, &deviceNums[i]);
		totalDeviceNum += deviceNums[i];
		printf("i=%d, deviceNums=%d\n",i,deviceNums[i]);
	}
	if (retCode != CL_SUCCESS)
	{
		printf("OKOKvoclProxyGetDeviceIDs, clGetDeviceIDs error %d\n", retCode);
		exit(1);
	}
	...
	/*
	{
		retCode |= clGetDeviceIDs(platformIDs[i], CL_DEVICE_TYPE_CPU, 
	...
	*/
	printf("OKOK\n");
	{
		retCode = clGetDeviceIDs(platformIDs[0], CL_DEVICE_TYPE_CPU, 
				1, &voclProxyDeviceIDs[0], NULL);
	}
	if (retCode != CL_SUCCESS)
	{
