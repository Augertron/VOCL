...
extern char* getKernelArgInfo(const char *kernel_name, vocl_program program, int *argNum);
...
cl_kernel clCreateKernel(cl_program program, const char *kernel_name, cl_int * errcode_ret)
{
    ...
    if (voclIsOnLocalNode(proxyIndex) == VOCL_TRUE) {
		...
		//printf("The tmpCreateKernel.argNum = %d\n");
                     ...
                     const cl_event * event_wait_list, cl_event * event)
{
	...
	if (voclIsOnLocalNode(proxyIndex) == VOCL_FALSE)
	{
    ...
    if (arg_value != NULL) {
        if (kernelPtr->args_flag[arg_index] == 1) {     /* device memory */
            ...
            memcpy(kernelPtr->args_ptr[kernelPtr->args_num].arg_value, (void *) &deviceMem,
                   arg_size);
			//printf("args_num = %d, deviceMem = %5d, arg_size = %5d\n",kernelPtr->args_num, deviceMem, arg_size);
                    ...
                    const cl_event * event_wait_list, cl_event * event)
{
    ...
    for (i = 0; i <= bufferNum; i++) {
        ...
        MPI_Irecv((void *) ((char *) ptr + VOCL_READ_BUFFER_SIZE * i), bufferSize, MPI_BYTE,
                  ...
                  //proxyRank, VOCL_READ_TAG, proxyCommData,
                      ...
                      void *param_value, size_t * param_value_size_ret)
{
    ...
    if (voclIsOnLocalNode(proxyIndex) == VOCL_TRUE) {
		...
		printf("program = %p, device = %p\n", tmpGetProgramBuildInfo.program, tmpGetProgramBuildInfo.device);
