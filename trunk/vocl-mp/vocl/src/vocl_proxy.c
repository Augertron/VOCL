...
//	int kernelLaunchThreadExecuting = 0;
	...
	//-------------------------------------
		...
		if (status.MPI_TAG == GET_PROXY_COMM_INFO) {
        	...
        	printf("Beg-CREATE_CONTEXT_FUNC\n");
        	...
        	printf("0-CREATE_CONTEXT_FUNC\n");
            mpiOpenCLCreateContext(&tmpCreateContext, devices);
        	printf("1-CREATE_CONTEXT_FUNC\n");
            ...
            MPI_Wait(curRequest, curStatus);
        	printf("4-CREATE_CONTEXT_FUNC\n");
            if (devices != NULL) {
                free(devices);
            }
        	printf("End-CREATE_CONTEXT_FUNC\n");
		}
