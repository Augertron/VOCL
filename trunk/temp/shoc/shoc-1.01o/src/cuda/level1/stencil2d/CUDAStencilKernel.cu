                ...
                T wDiagonal )
{
    ...
    int gidRow = blockIdx.y;
    int gidCol = blockIdx.x;
    int gszRow = gridDim.y;
    int gszCol = gridDim.x;
    int lidRow = threadIdx.y;
    int lidCol = threadIdx.x;
...
CUDAStencil<T>::operator()( Matrix2D<T>& mtx, unsigned int nIters )
{
    ...
    // we go from rows and columns to x,y coords,
    // where x indicates the column and y indicates the row
    dim3 dimGrid( gCols / LCOLS, gRows / LROWS );
    dim3 dimBlock( LCOLS, LROWS );
    ...
    size_t rowExtent = mtx.GetNumColumns() * sizeof(T);
    ...
    cudaMemcpy2D( newData + (mtx.GetNumRows() - 1) * mtx.GetNumColumns(),      // destination
                    rowExtent,  // destination pitch
                    currData + (mtx.GetNumRows() - 1) * mtx.GetNumColumns(),   // source
                    rowExtent,  // source pitch
                    rowExtent,  // width of data to transfer (bytes)
                    1,          // height of data to transfer (rows)
                    cudaMemcpyDeviceToDevice );
