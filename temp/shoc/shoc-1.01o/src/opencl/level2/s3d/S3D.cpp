...
//    h_molwt[21] = 2.80134E-02;
    ...
    // Extract out kernel objects
    cout << "Generating OpenCL Kernel Objects...";
    cout.flush();
    // GR Base Kernels
    ...
    for (unsigned int i = 0; i < passes; i++) {
        //Execute kernels
        Event evFirst("first kernel");
        Event evLast("last kernel");
        size_t globalWorkSize = n;
        size_t localWorkSize = 128;

        clLaunchKernelEv(grBase_kernel, evFirst.CLEvent());
        ...
        clLaunchKernel(ratx4_kernel);

        clLaunchKernel(qssa_kernel);
        ...
        clLaunchKernelEv(rdwdot10_kernel, evLast.CLEvent());
        err = clFinish(queue);
        CL_CHECK_ERROR(err);

        evFirst.FillTimingInfo();
        evLast.FillTimingInfo();
        double total = evLast.EndTime() - evFirst.StartTime();
