...
//	//===================================
	...
	for (subSequenceNo = 0; subSequenceNo < subSequenceNum; subSequenceNo++)
	{
		...
		timerStart();
		seq1D = clCreateBuffer(hContext, CL_MEM_READ_ONLY, sizeof(cl_char) * (rowNum - 1), 0, &err);
		CHECK_ERR(err, "Create seq1D memory");
		seq2D = clCreateBuffer(hContext, CL_MEM_READ_ONLY, sizeof(cl_char) * (columnNum - 1), 0, &err);
		...
		//timerStart();
		outSeq1D = clCreateBuffer(hContext, CL_MEM_READ_WRITE, sizeof(cl_char) * (rowNum + columnNum), 0, &err);
		CHECK_ERR(err, "Create outSeq1D memory");
		outSeq2D = clCreateBuffer(hContext, CL_MEM_READ_WRITE, sizeof(cl_char) * (rowNum + columnNum), 0, &err);
		...
		//timerStart();
		//threadNumD = clCreateBuffer(hContext, CL_MEM_READ_ONLY, sizeof(cl_int) * (rowNum + columnNum), 0, &err);
		//CHECK_ERR(err, "Create threadNumD memory");
		diffPosD = clCreateBuffer(hContext, CL_MEM_READ_ONLY, sizeof(cl_int) * (rowNum + columnNum), 0, &err);
