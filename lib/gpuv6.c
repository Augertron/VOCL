 ...
 * 6.	Try non-blocking message send instead of the blocking ones
 ****************************************************************************/
int slaveComm;
int slaveCreated = 0;
int np = 1;
...
{
	MPI_Request request;
	MPI_MYSEND(NULL, 0, MPI_BYTE, 0, PROGRAM_END, slaveComm, &request);
				 ...
				 cl_uint *        num_platforms)
{
	...
	MPI_MYSEND(&tmpGetPlatform, sizeof(tmpGetPlatform), MPI_BYTE, 0, 
			 GET_PLATFORM_ID_FUNC, slaveComm, &request);
               ...
               cl_uint *        num_devices)
{
	...
	MPI_MYSEND(&tmpGetDeviceIDs, sizeof(tmpGetDeviceIDs), MPI_BYTE, 0, 
			 GET_DEVICE_ID_FUNC, slaveComm, &request);
                ...
                cl_int *                       errcode_ret)
{
	...
	//send parameters to remote node
	res = MPI_MYSEND(&tmpCreateContext, sizeof(tmpCreateContext), MPI_BYTE, 0, 
			 CREATE_CONTEXT_FUNC, slaveComm, &request);
	if (res != MPI_SUCCESS)
	{
                     ...
                     cl_int *                       errcode_ret)
{
	...
	MPI_MYSEND(&tmpCreateCommandQueue, sizeof(tmpCreateCommandQueue), MPI_BYTE, 0, 
			 CREATE_COMMAND_QUEUE_FUNC, slaveComm, &request);
                          ...
                          cl_int *          errcode_ret)
{
	...
	//send parameters to remote node
	MPI_MYSEND(&tmpCreateProgramWithSource,
			 sizeof(tmpCreateProgramWithSource), 
			 MPI_BYTE, 0, CREATE_PROGRMA_WITH_SOURCE, 
			 slaveComm, &request1);
	MPI_MYSEND((void *)*strings, *lengths, MPI_BYTE, 0, 
			 CREATE_PROGRMA_WITH_SOURCE1, slaveComm, &request2);
               ...
               void *               user_data)
{
	...
	MPI_MYSEND(&tmpBuildProgram, sizeof(tmpBuildProgram), MPI_BYTE, 0, 
			 BUILD_PROGRAM, slaveComm, &request1);
	if (optionsLen > 0)
	{
		MPI_MYSEND((void *)options, optionsLen, MPI_CHAR, 0, BUILD_PROGRAM1, slaveComm, &request2);
	}
               ...
               cl_int *        errcode_ret)
{
	...
	MPI_MYSEND(&tmpCreateKernel, sizeof(tmpCreateKernel), MPI_BYTE, 0,
			 CREATE_KERNEL, slaveComm, &request1);
	MPI_MYSEND((void *)kernel_name, kernelNameSize, MPI_CHAR, 0, CREATE_KERNEL1, slaveComm, &request2);
               ...
               cl_int *     errcode_ret)
{
	...
	MPI_MYSEND(&tmpCreateBuffer, sizeof(tmpCreateBuffer), MPI_BYTE, 0, 
			 CREATE_BUFFER_FUNC, slaveComm, &request1);
	if (tmpCreateBuffer.host_ptr_flag == 1)
	{
		//printf("Here!\n");
		MPI_MYSEND(host_ptr, size, MPI_BYTE, 0, CREATE_BUFFER_FUNC1, slaveComm, &request2);
	}
                     ...
                     cl_event *         event)
{
	...
	MPI_MYSEND(&tmpEnqueueWriteBuffer, sizeof(tmpEnqueueWriteBuffer), MPI_BYTE, 0, 
			 ENQUEUE_WRITE_BUFFER, slaveComm, &request1);
	MPI_MYSEND((void *)ptr, cb, MPI_BYTE, 0, ENQUEUE_WRITE_BUFFER1, slaveComm, &request2);
               ...
               const void * arg_value)
{
	...
	MPI_MYSEND(&tmpSetKernelArg, sizeof(tmpSetKernelArg), MPI_BYTE, 0, 
			 SET_KERNEL_ARG, slaveComm, &request1);
	MPI_MYSEND((void *)arg_value, arg_size, MPI_BYTE, 0, SET_KERNEL_ARG1,
			 slaveComm, &request2);
                       ...
                       cl_event *       event)
{
	...
	MPI_MYSEND(&tmpEnqueueNDRangeKernel, sizeof(tmpEnqueueNDRangeKernel), MPI_BYTE, 0, 
			 ENQUEUE_ND_RANGE_KERNEL, slaveComm, &request1);
	...
	{
		MPI_MYSEND((void *)global_work_offset, sizeof(size_t) * work_dim, MPI_BYTE, 0,
				 ENQUEUE_ND_RANGE_KERNEL1, slaveComm, &request2);
	}

	if (tmpEnqueueNDRangeKernel.global_work_size_flag == 1)
	{
		MPI_MYSEND((void *)global_work_size, sizeof(size_t) * work_dim, MPI_BYTE, 0,
				 ENQUEUE_ND_RANGE_KERNEL2, slaveComm, &request3);
	}

	if (tmpEnqueueNDRangeKernel.local_work_size_flag == 1)
	{
		MPI_MYSEND((void *)local_work_size, sizeof(size_t) * work_dim, MPI_BYTE, 0,
				 ENQUEUE_ND_RANGE_KERNEL3, slaveComm, &request4);
	}
...
clReleaseMemObject(cl_mem memobj)
{
	...
	MPI_MYSEND(&tmpReleaseMemObject, sizeof(tmpReleaseMemObject), MPI_BYTE, 
			 0, RELEASE_MEM_OBJ, slaveComm, &request);
...
clReleaseKernel(cl_kernel kernel)
{
	...
	MPI_MYSEND(&tmpReleaseKernel, sizeof(tmpReleaseKernel), MPI_BYTE,
			 0, CL_RELEASE_KERNEL_FUNC, slaveComm, &request);
