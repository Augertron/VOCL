...
int runTest( const int argc, const char** argv) 
{
	...
	for (i = 0; i < 1; i++)
	{
    	ciErrNum  = clGetDeviceIDs(cpPlatforms[i], CL_DEVICE_TYPE_CPU, 0, NULL, &deviceNums[i]);
    	oclCheckError(ciErrNum, CL_SUCCESS);
	...
	//for (i = 0; i < platformNum; i++)
	{
    	//ciErrNum = clGetDeviceIDs(cpPlatforms[i], CL_DEVICE_TYPE_CPU, deviceNums[i], &cdDevices[uiNumDevices], NULL);
    	ciErrNum = clGetDeviceIDs(cpPlatforms[0], CL_DEVICE_TYPE_CPU, 1, &cdDevices[0], NULL);
    	oclCheckError(ciErrNum, CL_SUCCESS);
		//uiNumDevices += deviceNums[i];
	}
	timerEnd();
