...
void BodySystemCPU::_initialize(int numBodies)
{
    ...
    m_pos[0] = new double[m_numBodies*4];
	if (m_pos[0] == NULL)
	{
		printf("Allocate memory error!\n");
	}
    m_pos[1] = new double[m_numBodies*4];
    m_vel[0] = new double[m_numBodies*4];
    m_vel[1] = new double[m_numBodies*4];
    m_force  = new double[m_numBodies*4];

    memset(m_pos[0], 0, m_numBodies*4*sizeof(double));
    memset(m_pos[1], 0, m_numBodies*4*sizeof(double));
    memset(m_vel[0], 0, m_numBodies*4*sizeof(double));
    memset(m_vel[1], 0, m_numBodies*4*sizeof(double));
    memset(m_force, 0, m_numBodies*4*sizeof(double));
...
void BodySystemCPU::update(double deltaTime)
{
    oclCheckError(m_bInitialized, shrTRUE);
...
double* BodySystemCPU::getArray(BodyArray array)
{
    oclCheckError(m_bInitialized, shrTRUE);
...
void BodySystemCPU::setArray(BodyArray array, const double* data)
{
    oclCheckError(m_bInitialized, shrTRUE);
    double* target = 0;
    ...
    memcpy(target, data, m_numBodies*4*sizeof(double));
}

const double TOLERANCE = 3e-7f;
const double FUDGE_FACTOR = 0.025f;

extern void normalize(double vector[3]);
extern void bodyBodyInteraction(double accel[3], double posMass0[4], double posMass1[4], 
...
void BodySystemCPU::_integrateNBodySystem(double deltaTime)
{
...
double3 scalevec(double3& vector, double scalar)
{
    double3 rt = vector;
    rt.x *= scalar;
    rt.y *= scalar;
...
double normalize(double3& vector)
{
    double dist = sqrtf(vector.x*vector.x + vector.y*vector.y + vector.z*vector.z);
    if (dist > 1e-6)
    {
        vector.x /= dist;
	vector.y /= dist;
...
double dot(double3 v0, double3 v1)
{
...
// utility function
void randomizeBodies(NBodyConfig config, double* pos, double* vel, double* color, double clusterScale, double velocityScale, int numBodies)
{
    switch(config)
    {
    default:
    case NBODY_CONFIG_RANDOM:
        {
            double scale = clusterScale * MAX(1.0f, numBodies / (1024.f));
	    ...
	    while (i < numBodies) 
	    {
	        double3 point;
		//const int scale = 16;
		point.x = rand() / (double) RAND_MAX * 2 - 1;
		point.y = rand() / (double) RAND_MAX * 2 - 1;
		point.z = rand() / (double) RAND_MAX * 2 - 1;
                double lenSqr = dot(point, point);
                if (lenSqr > 1)
		    continue;
                double3 velocity;
    ...
    case NBODY_CONFIG_SHELL:
        {
            ...
            double vscale = scale * velocityScale;
            double inner = 2.5f * scale;
            double outer = 4.0f * scale;
            ...
            while (i < numBodies)//for(int i=0; i < numBodies; i++) 
            {
                ...
                double3 point = {x, y, z};
                double len = normalize(point);
                if (len > 1)
                    continue;

                pos[p++] =  point.x * (inner + (outer - inner) * rand() / (double) RAND_MAX);
                pos[p++] =  point.y * (inner + (outer - inner) * rand() / (double) RAND_MAX);
                pos[p++] =  point.z * (inner + (outer - inner) * rand() / (double) RAND_MAX);
                pos[p++] = 1.0f;
                
                x = 0.0f; // * (rand() / (double) RAND_MAX * 2 - 1);
                y = 0.0f; // * (rand() / (double) RAND_MAX * 2 - 1);
                z = 1.0f; // * (rand() / (double) RAND_MAX * 2 - 1);
                double3 axis = {x, y, z};
                normalize(axis);
                ...
                double3 vv = {pos[4*i], pos[4*i+1], pos[4*i+2]};
                vv = cross(vv, axis);
                vel[v++] = vv.x * vscale;
                vel[v++] = vv.y * vscale;
                vel[v++] = vv.z * vscale;
