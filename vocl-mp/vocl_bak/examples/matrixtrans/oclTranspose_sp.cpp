...
double transposeGPU(const char* kernelName, bool useLocalMem,  cl_uint ciDeviceCount, float* xh_idata, float* xh_odata, unsigned int size_x, unsigned int size_y, unsigned int pipeline)
{
	...
	for (iterNo = 0; iterNo < numIterations; iterNo++)
	{
		...
		{
			for(k = 0; k < pipeline; k++) {
				ciErrNum = clEnqueueWriteBuffer(commandQueues[i*pipeline+k], d_idata[i*pipeline+k], CL_FALSE, 0, mem_size, &h_idata[i*pipeline*size+size*k], 0, NULL, NULL);
				...
				ciErrNum |= clEnqueueNDRangeKernel(commandQueues[i * pipeline + k], ckKernels[i], 2, NULL, szGlobalWorkSize, szLocalWorkSize, 0, NULL, NULL);
				ciErrNum |= clEnqueueReadBuffer(commandQueues[i * pipeline + k], d_odata[i * pipeline + k], CL_FALSE, 0, mem_size, &h_odata[i*pipeline*size+size*k], 0, NULL, NULL);
				oclCheckError(ciErrNum, CL_SUCCESS);
			}
		}
		for (i = 0; i < ciDeviceCount; i++) {
			for(k = 0; k < pipeline; k++) {
				clFinish(commandQueues[i * pipeline + k ]);
			}
		}
	}
...
int runTest( const int argc, const char** argv) 
{
	...
	{
		index = i + deviceNo;
		for(m = 0; m <pipeline; m++) {
			commandQueues[i*pipeline+m] = clCreateCommandQueue(cxGPUContexts[i], cdDevices[index], CL_QUEUE_PROFILING_ENABLE, &ciErrNum);
			oclCheckError(ciErrNum, CL_SUCCESS);
		}
	}
	timerEnd();
    ...
    {
		for(m = 0; m < pipeline;m++) {
			ciErrNum = clReleaseCommandQueue(commandQueues[i*pipeline+m]);
			oclCheckError(ciErrNum, CL_SUCCESS);
		}
    }
	timerEnd();
