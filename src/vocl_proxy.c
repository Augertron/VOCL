...
void mpiOpenCLGetDeviceIDs(struct strGetDeviceIDs *tmpGetDeviceIDs, cl_device_id * devices)
{
    ...
    err = clGetDeviceIDs(platform, device_type, num_entries, devices, num_device_ptr);
...
void mpiOpenCLCreateBuffer(struct strCreateBuffer *tmpCreateBuffer, void *host_ptr)
{
    ...
    cl_mem deviceMem;
    deviceMem = clCreateBuffer(hInContext, flags, bufferSize, host_ptr, &err);
    if (err != CL_SUCCESS) {
...
void processWriteData(int dataReqNum, int totalReqNum, MPI_Request * request,
                      struct strDataInfo *dataInfo)
{
    ...
    for (i = 0; i < dataReqNum; i++) {
        ...
        DATA_TRANSFER *dataTransferPtr =
            createDataTransfer(dataInfo[index].writeBuffInfo.command_queue,
    ...
    while (1) {
        ...
        if (status.MPI_TAG == BUILD_PROGRAM) {
            ...
            if (tmpBuildProgram.device_list != NULL) {
                ...
                MPI_Irecv(device_list, sizeof(cl_device_id) * tmpBuildProgram.num_devices,
        ...
        if (status.MPI_TAG == CREATE_KERNEL) {
            ...
            char *kernelName =
                (char *) malloc((tmpCreateKernel.kernelNameSize + 1) * sizeof(char));
            MPI_Irecv(kernelName, tmpCreateKernel.kernelNameSize, MPI_CHAR, 0, CREATE_KERNEL1,
                      parentComm, curRequest);
        ...
        if (status.MPI_TAG == ENQUEUE_WRITE_BUFFER) {
            ...
            if (tmpEnqueueWriteBuffer.blocking_write == CL_TRUE) {
                ...
                processWriteData(dataReqNum, totalReqNum - CMSG_NUM, &conMsgRequest[CMSG_NUM],
                                 dataInfo);
                dataReqNum = 0;
                dataIndex = 0;
                totalReqNum = CMSG_NUM;
                ...
                DATA_TRANSFER *dataTransferPtr =
                    createDataTransfer(tmpEnqueueWriteBuffer.command_queue,
            ...
            {
                MPI_Irecv(host_ptr, tmpEnqueueWriteBuffer.cb, MPI_BYTE, 0,
                          tmpEnqueueWriteBuffer.tag, parentComm,
                          conMsgRequest + (CMSG_NUM + dataIndex));
                totalReqNum++;
                dataReqNum++;
                ...
                memcpy(&dataInfo[dataIndex].writeBuffInfo, &tmpEnqueueWriteBuffer,
                       sizeof(tmpEnqueueWriteBuffer));
        ...
        if (status.MPI_TAG >= ENQUEUE_WRITE_BUFFER + MIN_WRITE_TAG &&
            ...
            mpiOpenCLEnqueueWriteBuffer(&dataInfo[tempIndex].writeBuffInfo,
                                        dataInfo[tempIndex].host_ptr,
                                        dataInfo[tempIndex].event_wait_list);
            dataInfo[tempIndex].writeToGPUFlag = 1;
            DATA_TRANSFER *dataTransferPtr =
                createDataTransfer(dataInfo[tempIndex].writeBuffInfo.command_queue,
        ...
        if (status.MPI_TAG == ENQUEUE_READ_BUFFER) {
            ...
            if (num_events_in_wait_list > 0) {
                ...
                MPI_Irecv(event_wait_list, num_events_in_wait_list * sizeof(cl_event),
                          MPI_BYTE, 0, ENQUEUE_READ_BUFFER1, parentComm,
                          curRequest + (requestNo++));
            }
            ...
            if (tmpEnqueueReadBuffer.blocking_read == CL_FALSE) {
                ...
                DATA_TRANSFER *dataTransfer =
                    createDataTransfer(tmpEnqueueReadBuffer.command_queue,
        ...
        if (status.MPI_TAG == GET_KERNEL_WGP_INFO_FUNC) {
            ...
            MPI_Isend(&tmpGetKernelWorkGroupInfo, sizeof(tmpGetKernelWorkGroupInfo), MPI_BYTE,
                      0, GET_KERNEL_WGP_INFO_FUNC, parentComm, curRequest + (requestNo++));
