 ...
 * 11.  All are non-blocking memssage send and receive
 * 12.  every data block is stored separately
 ****************************************************************************/
//for slave process
int slaveComm;
int slaveCreated = 0;
...
MPI_Request *wrtCRequest, *wrtDRequest;
MPI_Request *readCRequest, *readDRequest;
...
DATA_READ *createDataRead(cl_command_queue command_queue, cl_event event)
{
    ...
    if (cmdQueue->dataReadPtr == NULL && cmdQueue->dataReadPtrTail == NULL) {
...
DATA_READ *getDataRead(cl_command_queue command_queue, cl_event event)
{
    CMD_QUEUE *cmdQueue = getCommandQueue(command_queue);
    ...
    if (dataReadPtr == NULL) {
        printf
            ("Lib, In function getDataRead() error, the corresponding event is not there!\n");
        exit(0);
    }
...
void releaseDataRead(cl_command_queue command_queue, cl_event event)
{
    CMD_QUEUE *cmdQueue = getCommandQueue(command_queue);
...
void processCommandQueue(cl_command_queue command_queue)
{
    ...
    //printf("inlib, Send time = %.3f, waittime = %.3f, requestNo = %d\n", sendTime, waitTime, requestNo);
    //-----------------------------------
...
void checkSlaveProc()
{
    if (slaveCreated == 0) {
        MPI_Init(NULL, NULL);
        MPI_Comm_spawn("/home/shucaixiao/workplace/trunk/lib/slave_process", MPI_ARGV_NULL, np,
        ...
        wrtCRequest =
            (MPI_Request *) malloc(sizeof(MPI_Request) * (MAX_WRITE_TAG - MIN_WRITE_TAG));
        wrtDRequest =
            (MPI_Request *) malloc(sizeof(MPI_Request) * (MAX_WRITE_TAG - MIN_WRITE_TAG));
        readCRequest =
            (MPI_Request *) malloc(sizeof(MPI_Request) * (MAX_READ_TAG - MIN_READ_TAG));
        readDRequest =
            (MPI_Request *) malloc(sizeof(MPI_Request) * (MAX_READ_TAG - MIN_READ_TAG));
        tmpEnqueueWriteBuffer =
            (struct strEnqueueWriteBuffer *) malloc((MAX_WRITE_TAG - MIN_WRITE_TAG) *
                                                    sizeof(struct strEnqueueWriteBuffer));
        tmpEnqueueReadBuffer =
            (struct strEnqueueReadBuffer *) malloc((MAX_READ_TAG - MIN_READ_TAG) *
                                                   sizeof(struct strEnqueueReadBuffer));
...
void mpiFinalize()
{
    ...
    free(readCRequest);
    free(readDRequest);
    free(tmpEnqueueWriteBuffer);
    free(tmpEnqueueReadBuffer);
}
                     ...
                     const cl_event * event_wait_list, cl_event * event)
{
    ...
    MPI_Status status[4];
    MPI_Request request[4];
    int writeIndex = nbWriteTag - MIN_WRITE_TAG;
    int requestNo = 0;

    //initialize structure
    tmpEnqueueWriteBuffer[writeIndex].command_queue = command_queue;
    tmpEnqueueWriteBuffer[writeIndex].buffer = buffer;
    tmpEnqueueWriteBuffer[writeIndex].blocking_write = blocking_write;
    tmpEnqueueWriteBuffer[writeIndex].offset = offset;
    tmpEnqueueWriteBuffer[writeIndex].cb = cb;
    tmpEnqueueWriteBuffer[writeIndex].tag = ENQUEUE_WRITE_BUFFER + nbWriteTag;
    tmpEnqueueWriteBuffer[writeIndex].num_events_in_wait_list = num_events_in_wait_list;
    if (event == NULL) {
        tmpEnqueueWriteBuffer[writeIndex].event_null_flag = 1;
    }
    else {
        tmpEnqueueWriteBuffer[writeIndex].event_null_flag = 0;
    }

    //send parameters to remote node
    MPI_Isend(&tmpEnqueueWriteBuffer[writeIndex], sizeof(struct strEnqueueWriteBuffer),
              MPI_BYTE, 0, ENQUEUE_WRITE_BUFFER, slaveComm, wrtCRequest + writeIndex);
    //MPI_Request_free(&request);
    if (num_events_in_wait_list > 0) {
        MPI_Isend((void *) event_wait_list, sizeof(cl_event) * num_events_in_wait_list,
                  MPI_BYTE, 0, tmpEnqueueWriteBuffer[writeIndex].tag, slaveComm,
                  request + (requestNo++));
        //MPI_Request_free(&request);
    }

    //MPI_Isend((void *)ptr, cb, MPI_BYTE, 0, ENQUEUE_WRITE_BUFFER+nbWriteTag, slaveComm, request+(requestNo++));
    MPI_Isend((void *) ptr, cb, MPI_BYTE, 0, tmpEnqueueWriteBuffer[writeIndex].tag, slaveComm,
              wrtDRequest + writeIndex);
    //MPI_Request_free(&request);

    if (blocking_write == CL_TRUE) {
        MPI_Irecv(&tmpEnqueueWriteBuffer[writeIndex], sizeof(struct strEnqueueWriteBuffer),
                  MPI_BYTE, 0, ENQUEUE_WRITE_BUFFER, slaveComm, request + (requestNo++));
        //for a blocking write, process all previous non-blocking ones
        processCommandQueue(command_queue);
        MPI_Waitall(requestNo, request, status);
        if (event != NULL) {
            *event = tmpEnqueueWriteBuffer[writeIndex].event;
        }
        return tmpEnqueueWriteBuffer[writeIndex].res;
    }
    else if (event != NULL) {
        MPI_Irecv(&tmpEnqueueWriteBuffer[writeIndex], sizeof(struct strEnqueueWriteBuffer),
                  MPI_BYTE, 0, ENQUEUE_WRITE_BUFFER, slaveComm, request + (requestNo));
                    ...
                    const cl_event * event_wait_list, cl_event * event)
{
    ...
    MPI_Status status[4];
    int readIndex = nbReadTag - MIN_READ_TAG;
    int requestNo = 0;
    //struct strEnqueueReadBuffer tmpEnqueueReadBuffer;

    //initialize structure
    tmpEnqueueReadBuffer[readIndex].command_queue = command_queue;
    tmpEnqueueReadBuffer[readIndex].buffer = buffer;
    tmpEnqueueReadBuffer[readIndex].blocking_read = blocking_read;
    tmpEnqueueReadBuffer[readIndex].readBufferTag = nbReadTag + ENQUEUE_READ_BUFFER;
    tmpEnqueueReadBuffer[readIndex].offset = offset;
    tmpEnqueueReadBuffer[readIndex].cb = cb;
    tmpEnqueueReadBuffer[readIndex].num_events_in_wait_list = num_events_in_wait_list;
    if (event == NULL) {
        tmpEnqueueReadBuffer[readIndex].event_null_flag = 1;
    }
    else {
        tmpEnqueueReadBuffer[readIndex].event_null_flag = 0;
    }

    //send parameters to remote node
    MPI_Isend(&tmpEnqueueReadBuffer[readIndex], sizeof(struct strEnqueueReadBuffer), MPI_BYTE,
              0, ENQUEUE_READ_BUFFER, slaveComm, readCRequest + readIndex);
    //MPI_Request_free(request);
    if (num_events_in_wait_list > 0) {
        MPI_Isend((void *) event_wait_list, sizeof(cl_event) * num_events_in_wait_list,
                  MPI_BYTE, 0, ENQUEUE_READ_BUFFER1, slaveComm, request);
        //MPI_Request_free(request);
    }

    if (blocking_read == CL_TRUE) {
        MPI_Irecv(&tmpEnqueueReadBuffer[readIndex], sizeof(struct strEnqueueReadBuffer),
                  MPI_BYTE, 0, ENQUEUE_READ_BUFFER, slaveComm, request + (requestNo++));
        MPI_Irecv(ptr, cb, MPI_CHAR, 0, ENQUEUE_READ_BUFFER1, slaveComm,
                  request + (requestNo++));
        //for a blocking read, process all previous non-blocking ones
        processCommandQueue(command_queue);
        MPI_Waitall(requestNo, request, status);
        if (event != NULL) {
            *event = tmpEnqueueReadBuffer[readIndex].event;
        }
        return tmpEnqueueReadBuffer[readIndex].res;
    }
    else        //non blocking read
    {
        if (event != NULL) {
            MPI_Irecv(&tmpEnqueueReadBuffer[readIndex], sizeof(struct strEnqueueReadBuffer),
                      MPI_BYTE, 0, ENQUEUE_READ_BUFFER, slaveComm, request);
        }
        MPI_Irecv(ptr, cb, MPI_CHAR, 0, nbReadTag + ENQUEUE_READ_BUFFER, slaveComm,
                  readDRequest + readIndex);

        DATA_READ *dataReadPtr = createDataRead(command_queue,
                                                tmpEnqueueReadBuffer[readIndex].event);
        dataReadPtr->request = readDRequest[readIndex];
        if (++nbReadTag >= MAX_READ_TAG) {
            nbReadTag == MIN_READ_TAG;
        }
